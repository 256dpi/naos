...
internal class SessionViewController: NSViewController {
	internal var device: DesktopDevice!
	...
	internal func process(
		title: String,
		operation: @escaping (NAOSSession, @escaping (Double, Double) -> Void) async throws
			-> Void
	) async {
		// show loading view controller
		let lvc = loadVC("LoadingViewController") as! LoadingViewController
		...
		let task = Task.detached {
			...
			try await operation(session) { progress, rate in
				DispatchQueue.main.async {
					...
					if rate > 0 {
						lvc.label.stringValue = String(
							format: title + "\n%.1f %% @ %.1f kB/s",
							progress * 100, rate / 1000)
					}
				}
			}
		}
