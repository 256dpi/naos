...
func Format(naosPath string, out io.Writer) error {
	...
	// prepare arguments
	arguments := []string{"-style", "{BasedOnStyle: Google, ColumnLimit: 120, SortIncludes: false, AllowShortFunctionsOnASingleLine: None}", "-i"}
	arguments = append(arguments, sourceFiles...)
	arguments = append(arguments, headerFiles...)

	// format source files
	err = Exec(naosPath, out, nil, false, false, "clang-format", arguments...)
	if err != nil {
		return err
	}
